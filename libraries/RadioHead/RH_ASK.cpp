...
void RH_ASK::timerSetup()
{
    ...
    uint8_t prescaler; // Bit values for CS0[2:0]
        ...
        if (!prescaler)
            return; // fault
    #if defined(RH_ATTINY_USE_TIMER0)
        #warning "Radio Head use Timer0 on ATtiny85"
        TCCR0A = 0;
        ...
        // Set mask to fire interrupt when OCF0A bit is set in TIFR0
            ...
            TIMSK |= _BV(OCIE0A);
        #endif
    #elif defined(RH_ATTINY_USE_TIMER1)
        #warning "Radio Head use Timer1 on ATtiny85"
        TCCR1 = 0;
        TCCR1 = _BV(CTC1) | prescaler; // Turn on CTC mode / Output Compare pins disconnected
                                       //  set CS10, CS11, CS12 to prescaler
                                       
        // Number of ticks to count before firing interrupt
        OCR1A = uint8_t(nticks);

        // Set mask to fire interrupt when OCF1A bit is set in TIFR
        #ifdef TIMSK0
            // ATtiny84
            TIMSK1 |= _BV(OCIE1A);
        #else
